...
const CITIES = ["دمشق", "حلب", "حمص", "حماة", "اللاذقية", "طرطوس", "دير الزور", "الحسكة"]
const CATEGORIES = ["شقة", "منزل", "أرض", "مكتب", "متجر", "مستودع"]
...
const AddProperty = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const fields = ['title', 'description', 'category', 'type', 'price', 'city', 'area', 'rooms', 'bathrooms', 'address', 'status', 'isFeatured']
      fields.forEach(f => {
  ...
  return (
    <div className="p-6 bg-[#f8faff] min-h-screen" dir="rtl">
      <Header title="إضافة عقار جديد" description="أدخل بيانات العقار الجديد بالكامل" />
      ...
      <form ref={formRef} onSubmit={handleSubmit} className="mt-8 bg-white rounded-[2rem] p-8 shadow-sm border border-gray-100">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="md:col-span-2 flex flex-col gap-1.5">
            ...
            <input name="title" required placeholder="مثال: شقة فاخرة في المزة"
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <input name="price" type="number" required min={0} placeholder="1,000,000"
          ...
          {/* المميزات */}
          <div className="md:col-span-2 flex flex-col gap-1.5">
            <label className="text-xs font-black text-gray-400">المميزات</label>
            <div className="flex gap-2">
              <input
                value={featureInput}
                onChange={e => setFeatureInput(e.target.value)}
                onKeyDown={e => e.key === 'Enter' && (e.preventDefault(), addFeature())}
                placeholder="مثال: مسبح، حديقة... ثم Enter"
              ...
              />
              <button type="button" onClick={addFeature} className="px-6 py-3 bg-blue-600 text-white rounded-2xl font-bold text-sm hover:bg-blue-700 transition">
                إضافة
              </button>
            </div>
            {features.length > 0 && (
              <div className="flex flex-wrap gap-2 mt-2">
                {features.map((f, i) => (
                  <span key={i} className="flex items-center gap-1 px-3 py-1.5 bg-blue-50 text-blue-700 rounded-xl text-xs font-bold">
                    {f}
                    <button type="button" onClick={() => removeFeature(i)} className="text-blue-400 hover:text-red-500 transition">
                      <MdClose size={14} />
                    </button>
                  </span>
                ))}
