...
const EditPropertyModal = ({ isOpen, onClose, property, onSuccess }: Props) => {
  ...
  const [showSuccess, setShowSuccess] = useState(false)  // ← جديد
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      fields.forEach(f => {
        if (formData[f] !== undefined) data.append(f, formData[f].toString())
      })
    ...
    <>
      <div className="fixed inset-0 z-[150] flex items-center justify-center p-4">
        ...
        <form
          ...
          className="relative bg-white w-full max-w-2xl rounded-[2.5rem] p-10 overflow-y-auto max-h-[90vh] shadow-2xl"
          dir="rtl"
        >
          {/* Header */}
          <div className="flex justify-between items-center mb-8">
            <div>
              <h2 className="text-2xl font-black text-[#0f2d4a]">تعديل بيانات العقار</h2>
              <p className="text-xs text-gray-400 font-bold mt-1">تحديث المعلومات الأساسية والصور</p>
            </div>
            <button type="button" onClick={onClose} className="p-3 bg-gray-50 text-gray-400 hover:text-red-500 rounded-2xl transition-all">
              <MdClose size={24} />
            </button>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="col-span-2 space-y-1.5">
              ...
              <div className="relative group">
                ...
                />
                <div className={`p-6 border-2 border-dashed rounded-[2rem] flex flex-col items-center gap-2 transition-all ${selectedFiles ? "border-blue-400 bg-blue-50" : "border-gray-200 group-hover:border-blue-400"
                  }`}>
                  <MdCloudUpload size={32} className={selectedFiles ? "text-blue-500" : "text-gray-300 group-hover:text-blue-500"} />
                  <span className="text-xs font-bold text-gray-400">
                    {selectedFiles ? `${selectedFiles.length} صور جاهزة للرفع — ستحل محل القديمة` : "اسحب الصور هنا أو اضغط للاختيار"}
                  </span>
                </div>
              </div>
            </div>
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full mt-8 py-5 bg-[#0f2d4a] text-white rounded-[1.5rem] font-black flex items-center justify-center gap-3 hover:bg-blue-900 shadow-xl transition-all disabled:opacity-50"
          >
            <MdSave size={24} />
