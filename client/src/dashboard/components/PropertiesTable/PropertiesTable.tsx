...
const PropertiesTable = ({ data, loading, refreshData }: Props) => {
  ...
  const handleCity = (city: string) => { setFilterCity(city); setCurrentPage(1) }
  ...
  return (
    <div className="mt-6 sm:mt-10 w-full" dir="rtl">
      ...
      {loading ? (
        <div className="py-16 text-center text-[11px] font-bold text-gray-300 animate-pulse">
          جاري جلب البيانات...
        </div>
      ) : (
        <>
            ...
            {currentData.map(p => (
              <div key={p._id} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-3 flex items-center gap-3">
                <div className="w-14 h-14 rounded-xl overflow-hidden shrink-0 border border-gray-100 bg-gray-50 flex items-center justify-center">
                  {getImgSrc(p.images)
                    ? <img src={getImgSrc(p.images)!} className="w-full h-full object-cover" alt={p.title} />
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-1.5">
                    ...
                    {p.price != null && <span className="text-[8px] text-gray-400 font-bold">ل.س</span>}
                    <span className={`w-1.5 h-1.5 rounded-full ${p.status === 'active' ? 'bg-green-500' : 'bg-gray-300'}`} />
                  </div>
                </div>
                <div className="shrink-0">
                  <PropertyActions propertyId={p._id} propertyTitle={p.title} propertyData={p} onActionSuccess={refreshData} />
                </div>
              </div>
            ))}
          </div>
              ...
              <tbody className="divide-y divide-gray-50">
                {currentData.map(p => (
                  <tr key={p._id} className="hover:bg-blue-50/10 transition-colors group">
                    ...
                    <td className="px-2 py-4 text-center">
                      <span className="text-[11px] font-black text-blue-600">{formatPrice(p.price)}</span>
                      {p.price != null && <span className="block text-[7px] text-gray-400 font-bold uppercase">ل.س</span>}
                    </td>
                    <td className="px-2 py-4 text-center">
